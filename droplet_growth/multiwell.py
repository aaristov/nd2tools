...
from scipy.ndimage import gaussian_filter, binary_erosion, binary_dilation, binary_fill_holes, label, measurements
...
def get_intensity_table(
    ...
    values: [str] = ['mean_intensity', 'intensity_image'],
    plot: bool = True
):
...
def get_mask(
    ...
    if post_dilate:
        good_mask = binary_dilation(good_mask, iterations=post_dilate)
